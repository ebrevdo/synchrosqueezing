...
%try
  ...
  if ~isempty(allAxes)
	...
	warning(['Skipping position. ' lasterr]);
        %disp(sprintf('Exception: %s',ME.message));
        %ME.stack,
      end
    else
      args = {args{:}, '-loose'};
...
tfig = ishandle(fig);
if length(fig)>0
    cellArray{length(fig),1}=[];
end
if length(tfig)>1
    cellArray(tfig) = get(fig(tfig),prop);
else
...
if (~isempty(value))
  ...
  inArrayt = ishandle(inArray);
  set(inArray(inArrayt),{prop},value(inArrayt));
end
...
try
  ...
  warning(['Skipping num2str. ' lasterr]);
end
